...
author: Angela Nolan and Prof Fisk
---
...
FILENAME<-("/Users/angelanolan/Desktop/week_7/breastFNA.csv")
...
all_rows <-1:569 #creates a vector of all the numbers between 1 and the number of rows 
new_vector <- sample(all_rows, size = 100, replace = FALSE,) #use sample function without replacement to randomly pick 100 row numbers and save it as a new vector 
print(new_vector)
...
A factor data type is a way to organize data based on categories, for example the data frame was organized by categories ID, diagnosis, radius, texture, perimiter, etc. 
...
mtry=31 had the best accuracy value
...
nomatch<-sum(trainDat$Prediction !=trainDat$Diagnosis)
print(nomatch)
...
there was a bias to the B lesion that was misclassified more often, as it was mismatched 4 times, compared to M which was not misclassified
...
I think it would be better for the algorithim to classify tumors benign tumors as malignant, but I think it would be helpful for the algorithim to be able to explain why the tumor was classified that way.
...
I would expect the accuracy to be 50%
...
##code using Adaptive-Network-Based Fuzzy Inference System model
if(!"caret"%in%installed.packages()){install.packages("caret", dependencies = TRUE)}
if(!"frbs"%in%installed.packages()){install.packages("frbs")}
library(caret)
library(frbs)
...
I thought it was cool to see the different outcomes when I tried the code using another model of my choosing.
