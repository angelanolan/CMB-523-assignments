...
"n" is the vector that we assigned the number to. the following line generates 83472047 random numbers that are between 1 and 2.
...
numberofruns <- 3 #number of times to run the code
runtimes <- numeric(numberofruns) #store time taken for each run
for (run in 1:numberofruns){
  start_time <- Sys.time() #records start time
  log_sum <- 0
...
end_time <- Sys.time() #records the end times
runtimes[run] <- end_time - start_time #calculate run time and store 
cat("Run", run, "completed in", runtimes[run], "seconds.\n")

#print the average of three runs
average_runtime <- mean(runtimes)
cat("Average runtime:", average_runtime, "seconds.\n")
```
...
ANSWER 2: i helps the variable "res" loop through the code. the first value it takes on is the first, second, 33rd, etc number(s) of the res vector, 
...
I helps run the loops in regards to the rows in the sequencing matrix, and j helps run the loops regarding the columns in the matrix
...
in the first version, nrow was called 1000 times (the ammount of rows in the sequening matrix), and ncol was called 2200 times (the ammount of collumns in the sequencing matrix )
...
Method 2 ran the fastest, and ran 57.12 times faster than the slowest method.
